...
declare i int default 1;
declare str varchar(100);
set str = ''; /*string is empty initailly*/
repeat
	set str = concat(str, i, ' ');
    set i = i + 1;
...
/*Write a stored procedure to fill the employee_name table 
with the data from the employees table. The name should be concatenated and then 
saved in the employee_name table*/
...
/*open the cursor, get the table form the database into cursor memory*/
open cur;

/*fetch the rows and put the values of each row in the variable*/
/*read_loop is the label for the loop so that we can exit using it*/
while not done do
